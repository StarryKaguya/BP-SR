...
class BPSRRefinementModel(SRModel):
    """Training wrapper for dual-stream same-resolution refinement."""
    
    def optimize_parameters(self, current_iter):
        self.optimizer_g.zero_grad()
        stacked_inputs = torch.stack([self.lq, self.diff], dim=1)
        # self.output = self.net_g(self.lq)
        self.output = self.net_g(stacked_inputs)
        l_total = 0
        ...
        if self.cri_pix: #CombinedLoss
            l_pix = self.cri_pix(self.gt, self.output)
            l_total += l_pix
            loss_dict['l_pix'] = l_pix
        # perceptual loss
    ...
    def pre_process(self):
        ...
        lq = F.pad(self.lq, (0, self.mod_pad_w, 0, self.mod_pad_h), 'reflect')
        diff = F.pad(self.diff, (0, self.mod_pad_w, 0, self.mod_pad_h), 'reflect')
        self.img = torch.stack([lq, diff], dim=1)
    ...
    def tile_process(self):
        ...
        batch, b2, channel, height, width = self.img.shape
