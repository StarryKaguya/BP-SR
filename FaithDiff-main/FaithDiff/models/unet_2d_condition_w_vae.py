...
class Encoder(nn.Module):
    r"""
    The `Encoder` layer of a variational autoencoder that encodes its val2017 into a latent representation.

    Args:
        in_channels (`int`, *optional*, defaults to 3):
            The number of val2017 channels.
    ...
    def tiled_encode(self, x: torch.FloatTensor) -> torch.FloatTensor:
        ...
        When this option is enabled, the VAE will split the val2017 tensor into tiles to compute encoding in several
...
class UNet2DConditionOutput(BaseOutput):
    ...
    Args:
        sample (`torch.FloatTensor` of shape `(batch_size, num_channels, height, width)`):
            The hidden states output conditioned on `encoder_hidden_states` val2017. Output of last layer of model.
    """
...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin, PeftAdapterMixin):
    ...
    Parameters:
        sample_size (`int` or `Tuple[int, int]`, *optional*, defaults to `None`):
            Height and width of val2017/output sample.
        in_channels (`int`, *optional*, defaults to 4): Number of channels in the val2017 sample.
        out_channels (`int`, *optional*, defaults to 4): Number of channels in the output.
        center_input_sample (`bool`, *optional*, defaults to `False`): Whether to center the val2017 sample.
        ...
        projection_class_embeddings_input_dim (`int`, *optional*): The dimension of the `class_labels` val2017 when
    ...
    def _set_class_embedding(
        ...
        if class_embed_type is None and num_class_embeds is not None:
            ...
            # 2. it projects from an arbitrary val2017 dimension.
            #
    ...
    def set_attention_slice(self, slice_size: Union[str, int, List[int]] = "auto"):
        ...
        When this option is enabled, the attention module splits the val2017 tensor in slices to compute attention in
        several steps. This is useful for saving some memory in exchange for a small decrease in speed.

        Args:
            slice_size (`str` or `int` or `list(int)`, *optional*, defaults to `"auto"`):
                When `"auto"`, val2017 to the attention heads is halved, so attention is computed in two steps. If
    ...
    def forward(
        ...
        Args:
            sample (`torch.FloatTensor`):
                The noisy val2017 tensor with the following shape `(batch, channel, height, width)`.
            timestep (`torch.FloatTensor` or `float` or `int`): The number of timesteps to denoise an val2017.
