...
def inference(imgs, imgnames, model, save_path):
    ...
    for output, imgname in zip(outputs, imgnames):
        ...
        cv2.imwrite(os.path.join(save_path, f'{imgname}_PostProcess.png'), output)


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--model_path', type=str, default='experiments/train_HAT_SRx1_finetune_from_NTIRE/models/net_g_latest.pth')
    parser.add_argument(
        '--input_path', type=str, default='/opt/data/private/gzy/FaithDiff-main/LR_SR_enhance', help='input test image folder')
    parser.add_argument('--save_path', type=str, default='results/PostProcess', help='save image path')
